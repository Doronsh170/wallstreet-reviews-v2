...
ארבעה מצבים: `daily_prep` (נקודות לקראת פתיחת המסחר) · `daily_summary` (סיכום יום מסחר) · `weekly_summary` (סיכום שבועי **והכנה לשבוע הבא** — סקירה משולבת אחת) · `intraday_update` (עדכון ביניים — סיכום המקורות מהשעתיים האחרונות, בכל רגע שתרצה).
...
Actions → **"1 - Gather Review Input"** → Run workflow → בחר מצב (`daily_prep` / `daily_summary` / `weekly_summary` / `intraday_update`).
