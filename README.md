...
ארבעה מצבים: `daily_prep` (נקודות לקראת פתיחת המסחר) · `daily_summary` (סיכום יום מסחר) · `weekly_summary` (סיכום שבוע המסחר שהסתיים) · `intraday_update` (עדכון ביניים — סיכום המקורות מהשעתיים האחרונות, בכל רגע שתרצה).
