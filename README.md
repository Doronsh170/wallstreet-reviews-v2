...
4. **מדביקים בשלב 3 ולוחצים "פרסם"** — לשלב 3 נכנסת אך ורק תשובת ה-JSON של הצ'אט.
   התיבה נפתחת ריקה בכל מחזור (וגם אחרי רענון הדף), וכפתור "פרסם" מושבת עד שהתוכן
   שהודבק הוא באמת payload של סקירה. הדבקה של חומר הגלם עצמו, של טקסט חופשי או של JSON
   קטוע נעצרת על המסך עם ההודעה "יש להדביק כאן את תשובת ה-JSON המלאה של Claude." —
   בלי לפנות לשרת ובלי להריץ אף guard.
   לחיצה חוזרת על "פרסם" עם אותו טקסט בדיוק לא כותבת קומיט ריק (כזה לא היה מפעיל את
   ה-workflow כלל, והמסך היה ממתין לריצה שלא תגיע): ה-Worker מפעיל את הפרסום ישירות
   ב-`workflow_dispatch`, וה-guards רצים שוב על אותו תוכן.
5. **אישור** — "הסקירה באוויר ✓", והתיבה מתרוקנת למחזור הבא. אם guard פסל את הסקירה,
   ההודעה שלו בעברית מופיעה על המסך עצמו (מה לתקן ואיך), הטקסט שהדבקת נשאר במקומו,
   מתקנים בצ'אט ומדביקים שוב.
