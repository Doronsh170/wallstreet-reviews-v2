...
ארבעה מצבים: `daily_prep` (הכנה ליום מסחר) · `daily_summary` (סיכום יום מסחר) · `weekly_summary` (סיכום שבועי) · `intraday_update` (עדכון ביניים — מה קרה בשווקים בשעתיים האחרונות, בכל רגע שתרצה).
