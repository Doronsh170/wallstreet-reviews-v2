...
שלושה מצבים: `daily_prep` (הכנה ליום מסחר) · `daily_summary` (סיכום יום מסחר) · `weekly_summary` (סיכום שבועי).
...
| `data.json` | מסד הנתונים של האתר: `dailyPrep` / `dailySummary` / `weeklySummary` + חגים |
...
Actions → **"1 - Gather Review Input"** → Run workflow → בחר מצב (`daily_prep` / `daily_summary` / `weekly_summary`).
...
1. הרץ: python gather_review_input.py <daily_prep / daily_summary / weekly_summary — לפי מה שאבקש>.
